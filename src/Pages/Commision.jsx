...
const Commission = () => {
    ...
    const getAllCommission = () => {
        setLoading(true);
        axios
            .get(`https://unioncooperativesocietylimited.in:8443/commisssion/getAllCommission/${currentpage}/${recordsperpages}/${selectedvalue}`)
            .then((response) => {
    ...
    const searchCustomers = () => {
        setLoading(true);
        axios
            .get('https://unioncooperativesocietylimited.in:8443/commisssion/findByCustomerIdAndDate', {
                params: {
                    customerId: custid,
                    localDate: date,
                },
    ...
    const searchByAccountNo = () => {
        ...
        if (searchAccNo) {
            axios
                .get(`https://unioncooperativesocietylimited.in:8443/commisssion/id/${searchAccNo}`)
                .then((response) => {
                                            ...
                                            return (
                                                <React.Fragment key={val.id}>
                                                    <tr style={{ fontSize: '20px' }}>
                                                        ...
                                                        <td style={{ fontSize: '20px' }}>{val.deductedAmount.toFixed(2)}</td>
                                                        <td style={{ fontSize: '20px' }}>{val.type}</td>
                                                        <td style={{ fontSize: '20px' }}>{val.withdrawalAmount.toFixed(2)}</td>
